...
export default async function AnalyticsPage({
  ...
  const d = db();
  // Apply the same scope the rest of the dashboard uses so the
  // Top Users / Top Repos / cost rollups reflect the active filter
  // instead of always showing the full team.
  ...
  // Subtitle reflects the active scope, matching the Runs/Sessions pages.
  let subtitle: string;
  if (scope.active === "user" && scope.userId) {
    const target = userById.get(scope.userId);
    subtitle = target
      ? `${target.name ?? target.email}'s metrics`
      : "Filtered metrics";
  } else if (scope.active === "mine") {
    subtitle = "Your metrics";
  } else {
    subtitle = "Aggregate metrics across all runs";
  }
